...
function renderHome(){
  ...
  hp.innerHTML=places.length?places.map(p=>`<div class="place"><div class="photo">${p.details?.photo_url?`<img src="${esc(p.details.photo_url)}" alt="" loading="lazy" onerror="this.remove()">`:'⌖'}<span>${p.details?.rating?`★ ${esc(p.details.rating)}`:'محفوظ'}</span></div><div class="p"><b>${esc(p.title)}</b><small>${esc(itemNotes(p)||t.city||'مكان محفوظ')}</small></div></div>`).join(''):'<div class="panel empty-state" style="min-width:100%">لا توجد أماكن محفوظة بعد. افتح «الأماكن» لطلب اقتراحات.</div>';
}
...
function renderPlaces(){
  ...
  box.innerHTML=items.map(i=>`<div class="place-row">
    ...
    <div>${i.details?.rating?`<div class="place-rating">★ ${esc(i.details.rating)}${i.details?.reviews?` · ${new Intl.NumberFormat('ar-AE',{notation:'compact'}).format(num(i.details.reviews))}`:''}</div>`:''}${i.details?.url?`<a class="mini-btn" href="${esc(i.details.url)}" target="_blank" rel="noopener">الخريطة</a>`:''}${`<button class="mini-btn" data-delete-item="${i.id}">حذف</button>`}</div>
  </div>`).join('');
}
...
async function requestSuggestions(){
  ...
  $('suggestBtn').disabled=true;msg($('suggestStatus'),'جاري تحليل التقييمات وآراء الزوار…');$('suggestionsList').innerHTML='';
  try{
    const data=await functionCall(CFG.functions.destinationSuggestions,{country,city});
    const items=data.items||[];
    msg($('suggestStatus'),items.length?'تم ترتيب الاقتراحات اعتمادًا على التقييم، عدد المراجعات وموثوقية الشعبية. الصور مأخوذة من بيانات المكان نفسها.':'لم يتم العثور على نتائج مناسبة.',items.length?'success':'');
    $('suggestionsList').innerHTML=items.map((p,idx)=>`<div class="suggestion">
      ...
      <div class="suggestion-body">
        <div class="suggestion-top"><div><h4>${esc(p.name)}</h4><p>${esc(p.category||'وجهة')} · ${esc(p.address||'')}</p></div><div class="rating">★ ${esc(p.rating||'—')} · ${new Intl.NumberFormat('ar-AE',{notation:'compact'}).format(num(p.reviews))} رأي</div></div>
        <div class="suggestion-actions"><button class="add" data-add-suggest="${idx}">+ أضف للرحلة</button>${p.url?`<a href="${esc(p.url)}" target="_blank" rel="noopener">Google Maps</a>`:''}</div>
      </div>
    </div>`).join('');
    qsa('[data-add-suggest]').forEach(b=>b.onclick=()=>addSuggestion(items[Number(b.dataset.addSuggest)]));
  }catch(e){
    const providerMissing=e?.message==='RATINGS_PROVIDER_NOT_CONFIGURED';
    msg($('suggestStatus'),providerMissing?'خدمة تقييمات Google Places غير مفعلة على الخادم بعد. يلزم مفتاح Google Places لعرض تقييم الناس والصور الدقيقة.':'تعذر جلب الاقتراحات حاليًا. حاول مرة أخرى.','error');
...
async function addSuggestion(p){
  ...
  const details={notes:[p.category,p.address].filter(Boolean).join(' · '),rating:p.rating,reviews:p.reviews,url:p.url,source:p.source,photo_url:p.photo_url||'',place_id:p.id||''};
  ...
  await loadItems(state.currentTrip.id);renderAll();alert('تمت إضافة المكان مع صورته وتقييمه إلى الرحلة.');
}
...
function bindStaticHandlers(){
  ...
  $('saveTripBtn').onclick=saveTrip;$('saveItemBtn').onclick=saveItem;$('suggestBtn').onclick=requestSuggestions;
