...
const state = { session:null, profile:null, trips:[], currentTrip:null, items:[], releases:[], notifications:[], notificationChannel:null, releaseChannel:null, swRegistration:null };
...
async function fetchCityCover(city,country){
  ...
  try{const data=await functionCall(CFG.functions.destinationSuggestions,{mode:'city-cover',city,country});return data?.photo_url||''}catch(e){console.warn('city cover',e);return ''}
}
...
async function deleteTrip(id){
  ...
  if(wasCurrent)localStorage.removeItem('rahalati-current-trip');
  await loadTrips();
...
async function requestSuggestions(){
  ...
  $('suggestBtn').disabled=true;msg($('suggestStatus'),'جاري جمع الأماكن البارزة من المصادر المفتوحة…');$('suggestionsList').innerHTML='';
  try{
    const data=await functionCall(CFG.functions.destinationSuggestions,{mode:'places',country,city});
    const items=data.items||[];
    msg($('suggestStatus'),items.length?'تم ترتيب الأماكن اعتمادًا على شهرتها ووجودها في Wikipedia/Wikidata، مع صور من Wikimedia. لا توجد رسوم أو مفاتيح API مدفوعة.':'لم يتم العثور على نتائج مناسبة.',items.length?'success':'');
    $('suggestionsList').innerHTML=items.map((p,idx)=>`<div class="suggestion">
      <div class="suggestion-media">${p.photo_url?`<img src="${esc(p.photo_url)}" alt="${esc(p.name)}" loading="lazy" onerror="this.remove()">`:'<div class="photo-fallback">⌖</div>'}</div>
      <div class="suggestion-body">
        <div class="suggestion-top"><div><h4>${esc(p.name)}</h4><p>${esc(p.category||'وجهة')} · ${esc(p.address||[city,country].filter(Boolean).join('، '))}</p></div><div class="community-score">مؤشر ${esc(p.community_score||'—')}%</div></div>
        <p class="source-note">${esc(p.popularity_label||'مصدر مفتوح')}</p>
  ...
  }catch(e){msg($('suggestStatus'),'تعذر جلب الاقتراحات المفتوحة حاليًا. حاول مرة أخرى.','error')}
